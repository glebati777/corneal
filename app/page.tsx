...
const STORAGE_KEY = "corneal_platform_v4_research";
...
function clinicalSuggestions(patient?: Patient) {
  ...
  if (suggestions.length === 0) suggestions.push("Сохранять стандартное плановое наблюдение с повторной оценкой по протоколу.");
...
export default function Page() {
  ...
  const [simIL1, setSimIL1] = useState(0);
  const [simIL6, setSimIL6] = useState(0);
  const [simTNF, setSimTNF] = useState(0);
  const [simVEGF, setSimVEGF] = useState(0);
  const [simTGFB, setSimTGFB] = useState(0);
    ...
    <div className="page">
      <aside className="sidebar">
        ...
        <div className="nav">
          ...
          <button className={`navBtn ${page === "simulation" ? "active" : ""}`} onClick={() => setPage("simulation")}><SlidersHorizontal size={16} />Симуляция</button>
          <button className={`navBtn ${page === "research" ? "active" : ""}`} onClick={() => setPage("research")}><FlaskConical size={16} />Исследование</button>
      ...
      <main className="main">
        <div className="container">
          <div className="hero heroV4 glass">
            <div className="heroBadge"><HeartPulse size={14} />Corneal Graft Analytics V5</div>
            <div className="heroTopLine"><h1 className="heroTitle">Clinical AI + research mode V5</h1><div className="heroMiniPanel"><div className="muted tiny">Текущий прогноз</div><strong>{ml.probability}%</strong></div></div>
            <div className="heroText">Режим симуляции риска, cohort analytics, исследовательские панели и более зрелая продуктовая логика для медицинского AI-интерфейса.</div>
            ...
            <div className="grid g3">
              ...
              <div className="card glass">
                ...
                <div className="grid" style={{ marginTop: 18 }}>
                  ...
                  <button className="btn outline" onClick={() => setPage("simulation")}><SlidersHorizontal size={16} />Режим симуляции</button>
                </div>
              </div>
            </div>
          )}
