...
const STORAGE_KEY = "corneal_platform_v2_visits";
...
function calcRisk(patient?: Patient) {
  ...
  if (!patient || !visit) return { risk: 0, level: "Низкий", status: "Стабильное наблюдение" };
...
function calcMlForecast(patient?: Patient) {
  ...
  const x =
    ...
    visit.il1 * 0.05 + visit.il6 * 0.06 + visit.tnf * 0.05 + visit.vegf * 0.012 + visit.tgfb * 0.01;
...
function exportReport(patient?: Patient) {
  ...
  const html = `
    <html><head><title>Клинический отчёт</title><style>
    ...
    <div class="grid">
      <div class="box"><strong>Пациент</strong><div>${patient.fullName}</div><div>ID: ${patient.id}</div></div>
      <div class="box"><strong>Дата визита</strong><div>${visit.date}</div><div>${surgeryLabel(patient.surgeryType)}</div></div>
      <div class="box"><strong>Оценка риска</strong><div>${risk.risk}% · ${risk.level}</div></div>
      <div class="box"><strong>Вероятность отторжения</strong><div>${ml.probability}% · ${ml.classLabel}</div></div>
    </div>
    <div class="box"><strong>Биомаркеры</strong><div>IL-1 ${visit.il1} · IL-6 ${visit.il6} · TNF-α ${visit.tnf} · VEGF ${visit.vegf} · TGF-β ${visit.tgfb}</div></div>
    <div class="box" style="margin-top:12px"><strong>Клиническая интерпретация</strong><div style="margin-top:8px">${makeExplanation(patient)}</div></div>
...
function RiskGauge({ value, label }: { value: number; label: string }) {
  ...
  return (
    <div className="gaugeWrap">
      <div className={`gaugeShell ${tone}`}>
        <div className="gaugeArc" />
        <div className="gaugeInner" />
        <div className="gaugeNeedle" style={{ ["--angle" as any]: angle }} />
        <div className="gaugeDot" />
        <div className="gaugeCenterText"><div className="gaugeValue">{value}%</div><div className="gaugeLabel">{label}</div></div>
        <div className="gaugeLabels"><span>НИЗКИЙ</span><span>УМЕРЕННЫЙ</span><span>ВЫСОКИЙ</span></div>
      </div>
    </div>
  );
}
...
function TrendChart({ title, visits, getValue }: { title: string; visits: Visit[]; getValue: (v: Visit) => number; }) {
  ...
  return (
    <div className="trendCard glass">
      <div className="trendHeader"><div><div className="muted tiny">{title}</div><div className="trendValue">{values[values.length - 1] ?? 0}</div></div><LineChart size={18} /></div>
      <svg viewBox="0 0 100 100" className="trendSvg" preserveAspectRatio="none">
        <defs><linearGradient id={`g-${title}`} x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="rgba(16,185,129,1)" /><stop offset="100%" stopColor="rgba(59,130,246,1)" /></linearGradient></defs>
        <polyline fill="none" stroke={`url(#g-${title})`} strokeWidth="4" points={points} strokeLinecap="round" strokeLinejoin="round" />
      </svg>
      <div className="trendDates">{ordered.map((v) => <span key={v.id}>{v.date.slice(5)}</span>)}</div>
    </div>
  );
}
...
function LoginScreen({ onLogin }: { onLogin: (u: { email: string }) => void }) {
  ...
  return (
    <div style={{ minHeight: "100vh", padding: 24 }}>
      <div className="container shellCenter">
        <div className="grid g2">
          <div className="card glass">
            <div className="brand" style={{ marginBottom: 16 }}>
              <div className="brandIcon"><Eye size={24} /></div>
              <div><div className="titleXL">Corneal Risk Platform</div><div className="brandSub">Clinical intelligence interface</div></div>
            </div>
            <div className="grid">
            ...
            <div style={{ marginTop: 20 }}>
              <button className="btn" style={{ width: "100%" }} onClick={() => onLogin({ email })}><LogIn size={16} />Войти в систему</button>
            </div>
          </div>
          <div className="hero heroTall glass">
            <div className="heroBadge"><Sparkles size={14} />V2 Clinical Experience</div>
            <h1 className="heroTitle">Наблюдение в динамике</h1>
            <div className="heroText">Apple-уровень интерфейса: аккуратная структура, динамика биомаркеров, повторные визиты пациента, автоматический перерасчёт риска и демонстрационный прогноз вероятности отторжения.</div>
          </div>
        </div>
      </div>
    </div>
...
export default function Page() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      high: risks.filter((r) => r >= 65).length,
      moderate: risks.filter((r) => r >= 35 && r < 65).length,
      avg: risks.length ? Math.round(risks.reduce((a, b) => a + b, 0) / risks.length) : 0,
    };
  }, [patients]);

  const filteredPatients = useMemo(() => {
    let arr = [...patients].filter((p) =>
      [p.id, p.fullName, getLatestVisit(p)?.notes || ""].some((v) =>
  ...
  const savePatient = () => {
    ...
    const newPatient: Patient = {
      ...patientForm,
      visits: [{ ...emptyVisit, id: `V-${patientForm.id}-1`, notes: "Первичный визит после внесения пациента в систему." }],
    };
  ...
  return (
    <div className="page">
      <aside className="sidebar">
        <div className="brand">
          <div className="brandIcon"><Eye size={24} /></div>
          <div><div className="brandTitle">Corneal Risk</div><div className="brandSub">Clinical Decision Support</div></div>
        </div>
        ...
        <div className="userBox glass">
          <div style={{ fontWeight: 700, color: "#fff" }}>Пользователь</div>
          <div style={{ marginTop: 6 }}>{user.email}</div>
          <div className="tiny" style={{ marginTop: 12 }}>Оценка риска и прогноз вероятности отторжения в этой версии остаются демонстрационными.</div>
        </div>
      </aside>

      <main className="main">
        <div className="container">
          <div className="hero heroV2 glass">
            <div className="heroBadge"><HeartPulse size={14} />Corneal Graft Analytics V2</div>
            <div className="heroTopLine">
              <h1 className="heroTitle">Платформа наблюдения в динамике</h1>
              <div className="heroMiniPanel"><div className="muted tiny">Текущий прогноз</div><strong>{ml.probability}%</strong></div>
            </div>
            <div className="heroText">Премиальный интерфейс с динамикой биомаркеров, повторными визитами, автоматическим перерасчётом риска и AI-объяснением клинической картины.</div>
            ...
            <div className="grid g3">
              <div className="card glass" style={{ gridColumn: "span 2" }}>
                <div className="sectionTitle">
                  <div>
                    <h2>Центральная клиническая карточка V2</h2>
                    <div className="muted small">Данные пациента, динамика маркеров, перерасчёт риска и интерпретация в одной панели.</div>
                  </div>
                  <div className="kpiPill"><ShieldCheck size={14} />Динамический перерасчёт</div>
                </div>

                <div className="grid g2" style={{ marginTop: 18 }}>
                  <div className="grid">
                    <div className="panel premiumPanel glass">
                      <div className="headerRow">
                        <div>
                          <div className="muted small">Активный пациент</div>
                          <div className="titlePatient">{selected.fullName}</div>
                          <div className="muted small">{selected.id} · {selected.age} лет · визит {latestVisit.date}</div>
                        </div>
                    ...
                    <div className={`alertBox ${alertTone}`}>
                      <div className="alertTitle"><AlertTriangle size={18} />Клиническое предупреждение</div>
                      <div className="small" style={{ marginTop: 8 }}>
                        {selectedRisk.risk >= 65 ? "Высокая ориентировочная вероятность осложнений. Рекомендуется усиленный мониторинг." : selectedRisk.risk >= 35 ? "Есть факторы, требующие более частого наблюдения." : "Текущая картина соответствует стабильному плановому наблюдению."}
                      </div>
                    </div>

                    <div className="softCard glass">
                      <div className="headerRow"><div><div className="muted tiny">AI-интерпретация</div><strong>Что меняется между визитами</strong></div><Sparkles size={18} /></div>
                      <div className="small muted" style={{ marginTop: 10 }}>{makeExplanation(selected)}</div>
                    </div>
                  </div>
                  ...
                  <div className="grid">
                    <div className="panel premiumTopGrid glass">
                      ...
                      <div className="softCard glass compactInfoCard">
                        <div className="headerRow"><div><div className="muted tiny">Вероятность отторжения</div><strong style={{ fontSize: 34 }}>{ml.probability}%</strong><div className="small muted">{ml.classLabel}</div></div><Cpu size={18} /></div>
                        <div className="small muted" style={{ marginTop: 10 }}>Модель пересчитывается по последнему визиту пациента и обновляет оценку автоматически.</div>
                      </div>
                    </div>
                    ...
                    <div className="grid g3">
                      {heatItems.map(([name, value]) => {
                        ...
                        return (
                          <div key={name} className="heatItem glass">
                            <div className="heatHeader"><span>{name}</span><span className={`statusDot status-${s}`} /></div>
                            <div className="heatValue">{value}</div>
                            <div className="tiny muted">{s === "high" ? "Высокий" : s === "mid" ? "Промежуточный" : "Низкий"}</div>
                          </div>
                        );
                      })}
                    </div>
              ...
              <div className="card glass">
                ...
                <div className="muted small">Управление визитами и быстрый доступ к функциям системы.</div>
          ...
          {page === "patients" && selected && latestVisit && (
            <div className="grid g3">
              <div className="card glass">
                ...
                <div className="muted small">Сортировка по риску, быстрый поиск и навигация по динамике.</div>
                ...
                <div className="filters">
                  ...
                  <select className="select" value={riskFilter} onChange={(e) => setRiskFilter(e.target.value)}>
                    <option value="all">Все уровни риска</option><option value="low">Низкий риск</option><option value="mid">Умеренный риск</option><option value="high">Высокий риск</option>
                  </select>
                  <select className="select" value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
                    <option value="risk-desc">Риск: по убыванию</option><option value="risk-asc">Риск: по возрастанию</option><option value="name">По имени</option><option value="visit">По дате визита</option>
                  </select>
                    ...
                    return (
                      <button key={patient.id} className={`patientBtn ${selectedId === patient.id ? "active" : ""}`} onClick={() => setSelectedId(patient.id)}>
                        <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
                          <div><div className="patientName">{patient.fullName}</div><div className="muted small">{patient.id} · {patient.surgeryType} · визит {visit?.date}</div></div>
                          <span className={`badge ${risk.risk >= 65 ? "high" : risk.risk >= 35 ? "mid" : "low"}`}>{risk.risk}%</span>
                        </div>
                        <div className="muted small" style={{ marginTop: 8 }}>{visit?.notes}</div>
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="card glass" style={{ gridColumn: "span 2" }}>
                <div className="headerRow">
                  <div><h2>Карточка пациента</h2><div className="muted small">Повторные визиты, динамика маркеров и автоматический перерасчёт риска.</div></div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <button className="btn outline" onClick={() => setVisitModal(true)}>Добавить визит</button>
                    <button className="btn danger" onClick={() => removePatient(selected.id)}><Trash2 size={16} />Удалить</button>
                  </div>
                </div>
                ...
                {tab === "summary" && (
                  <div className="grid g3" style={{ marginTop: 18 }}>
                    {[
                      ...
                      <div key={String(label)} className="panel glass"><div className="muted small">{label}</div><div style={{ marginTop: 6, fontWeight: 700 }}>{value}</div></div>
                    ))}
                ...
                {tab === "markers" && (
                  <div className="grid g2" style={{ marginTop: 18 }}>
                    ...
                    <div className="card glass">
                      <div className="muted small">История визитов</div>
                      <div className="timelineList" style={{ marginTop: 12 }}>
                        {[...selected.visits].sort((a, b) => b.date.localeCompare(a.date)).map((v) => (
                          <div key={v.id} className="timelineItem">
                            <div className="timelineDate">{v.date}</div>
                            <div><div className="timelineTitle">IL-1 {v.il1} · IL-6 {v.il6} · TNF-α {v.tnf}</div><div className="small muted" style={{ marginTop: 4 }}>{v.notes}</div></div>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                {tab === "recommendations" && (
                  <div className="grid g2" style={{ marginTop: 18 }}>
                    <div className="grid">
                      ...
                      <div className="softCard glass">
                        <div className="headerRow"><div><div className="muted tiny">AI-объяснение</div><strong>Почему риск изменился</strong></div><Activity size={18} /></div>
                        <div className="small muted" style={{ marginTop: 8 }}>{makeExplanation(selected)}</div>
                      </div>
                    </div>

                    <div className="grid">
                      <div className="softCard glass">
                        <div className="headerRow"><div><div className="muted tiny">Вероятность отторжения</div><strong>{ml.probability}%</strong></div><Cpu size={18} /></div>
                        <div className="progress" style={{ marginTop: 12 }}><div className="bar" style={{ width: `${ml.probability}%` }} /></div>
                        <div className="small muted" style={{ marginTop: 8 }}>{ml.classLabel}</div>
                      </div>
                      <div className="card glass">
                        <div className="muted small">Рекомендации</div>
                        <div className="recoList">{recommendations(selected).map((r, i) => <div key={i} className="recoItem"><span>{r}</span><strong>•</strong></div>)}</div>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {tab === "report" && (
                  <div className="reportSheet" style={{ marginTop: 18 }}>
                    ...
                    <div className="reportMeta">
                      <div className="reportBlock"><strong>Пациент</strong><div>{selected.fullName}</div><div>ID: {selected.id}</div></div>
                      <div className="reportBlock"><strong>Дата визита</strong><div>{latestVisit.date}</div><div>{surgeryLabel(selected.surgeryType)}</div></div>
                      <div className="reportBlock"><strong>Оценка риска</strong><div>{selectedRisk.risk}% · {selectedRisk.level}</div></div>
                      <div className="reportBlock"><strong>Вероятность отторжения</strong><div>{ml.probability}% · {ml.classLabel}</div></div>
                    </div>
                    <div className="reportBlock" style={{ marginTop: 14 }}><strong>Клиническая интерпретация</strong><div style={{ marginTop: 8 }}>{makeExplanation(selected)}</div></div>
                    <div style={{ marginTop: 16 }}><button className="btn" onClick={() => exportReport(selected)}><Download size={16} />Печать / сохранить PDF</button></div>
                  </div>
                )}
              </div>
            </div>
          ...
          {page === "analytics" && (
            <div className="grid g2">
              ...
              <div className="card glass">
                <h2>AI и ML блоки</h2>
                <div className="note" style={{ marginTop: 14 }}>В версии V2 прогнозные блоки пересчитываются по последнему визиту пациента. Это демонстрационная логика, которую затем можно заменить на реальную модель при наличии клинического датасета.</div>
              </div>
            </div>
          )}
            ...
            <div className="grid g2">
              <div className="card glass">
                ...
                <div className="grid small muted" style={{ marginTop: 18 }}>
                  <div>Платформа показывает не только текущий срез, но и динамику маркеров по повторным визитам.</div>
                  <div>При добавлении нового визита происходит автоматический перерасчёт риска и прогноза вероятности отторжения.</div>
                  <div>Текущая модель остаётся демонстрационной и не заменяет решение врача.</div>
                </div>
              </div>
              <div className="card glass">
                <h2>Что важно для следующего этапа</h2>
                <div className="grid small muted" style={{ marginTop: 18 }}>
                  <div>Подключение настоящего backend и базы данных визитов.</div>
                  <div>Импорт лабораторных значений и автоматическое заполнение маркеров.</div>
                  <div>Внешняя валидация ML-модели на реальных клинических данных.</div>
                </div>
              </div>
            </div>
          )}
          ...
          {page === "about" && (
            <div className="grid g2">
              <div className="card glass">
                <h2>О платформе</h2>
                <div className="grid small muted" style={{ marginTop: 18 }}>
                  <div>Corneal Risk Platform V2 объединяет карточку пациента, историю визитов, графики динамики и автоматическую интерпретацию данных.</div>
                  <div>Интерфейс создавался как продуктовый демонстрационный прототип с более дорогой и современной подачей.</div>
                </div>
              </div>
              <div className="card glass">
                <h2>Назначение</h2>
                <div className="grid small muted" style={{ marginTop: 18 }}>
                  <div>Учебная, исследовательская и демонстрационная работа с клиническими сценариями наблюдения.</div>
                  <div>Подготовка логики для следующего шага — реальной платформы с backend, базой данных и ML-моделью.</div>
                </div>
              </div>
            </div>
          )}
      ...
      {patientModal && (
        <div className="modalBg">
          <div className="modal glass">
            <div className="headerRow">
              <div><h2 style={{ margin: 0, color: "#fff" }}>Новый пациент</h2><div className="muted small">Создание карточки пациента для дальнейшего наблюдения.</div></div>
              <button className="btn outline" onClick={() => setPatientModal(false)}><X size={16} /></button>
            </div>
            <div className="grid g3" style={{ marginTop: 20 }}>
              <div><label className="label">ID пациента</label><input className="input" value={patientForm.id} onChange={(e) => setPatientForm({ ...patientForm, id: e.target.value })} /></div>
              <div><label className="label">ФИО</label><input className="input" value={patientForm.fullName} onChange={(e) => setPatientForm({ ...patientForm, fullName: e.target.value })} /></div>
              <div><label className="label">Возраст</label><input className="input" type="number" value={patientForm.age} onChange={(e) => setPatientForm({ ...patientForm, age: Number(e.target.value) })} /></div>
              <div><label className="label">Пол</label><select className="select" value={patientForm.sex} onChange={(e) => setPatientForm({ ...patientForm, sex: e.target.value as "М" | "Ж" })}><option value="М">М</option><option value="Ж">Ж</option></select></div>
              <div><label className="label">Тип трансплантации</label><select className="select" value={patientForm.graftType} onChange={(e) => setPatientForm({ ...patientForm, graftType: e.target.value as "primary" | "repeat" })}><option value="primary">Первичная</option><option value="repeat">Повторная</option></select></div>
              <div><label className="label">Вид операции</label><select className="select" value={patientForm.surgeryType} onChange={(e) => setPatientForm({ ...patientForm, surgeryType: e.target.value as Patient["surgeryType"] })}><option value="PKP">PKP</option><option value="DALK">DALK</option><option value="DSAEK">DSAEK</option><option value="DMEK">DMEK</option></select></div>
            </div>
            <div style={{ display: "flex", justifyContent: "end", gap: 10, marginTop: 18 }}>
              <button className="btn outline" onClick={() => setPatientModal(false)}>Отмена</button>
              <button className="btn" onClick={savePatient}>Сохранить пациента</button>
            </div>
          </div>
        </div>
      )}
      ...
      {visitModal && selected && (
        <div className="modalBg">
          <div className="modal glass">
            <div className="headerRow">
              <div><h2 style={{ margin: 0, color: "#fff" }}>Новый визит</h2><div className="muted small">Добавление новых значений маркеров с автоматическим перерасчётом риска.</div></div>
              <button className="btn outline" onClick={() => setVisitModal(false)}><X size={16} /></button>
            </div>

            <div className="grid g3" style={{ marginTop: 20 }}>
              <div><label className="label">Дата визита</label><input className="input" type="date" value={visitForm.date} onChange={(e) => setVisitForm({ ...visitForm, date: e.target.value })} /></div>
              <div><label className="label">IL-1</label><input className="input" type="number" value={visitForm.il1} onChange={(e) => setVisitForm({ ...visitForm, il1: Number(e.target.value) })} /></div>
              <div><label className="label">IL-6</label><input className="input" type="number" value={visitForm.il6} onChange={(e) => setVisitForm({ ...visitForm, il6: Number(e.target.value) })} /></div>
              <div><label className="label">TNF-α</label><input className="input" type="number" value={visitForm.tnf} onChange={(e) => setVisitForm({ ...visitForm, tnf: Number(e.target.value) })} /></div>
              <div><label className="label">VEGF</label><input className="input" type="number" value={visitForm.vegf} onChange={(e) => setVisitForm({ ...visitForm, vegf: Number(e.target.value) })} /></div>
              <div><label className="label">TGF-β</label><input className="input" type="number" value={visitForm.tgfb} onChange={(e) => setVisitForm({ ...visitForm, tgfb: Number(e.target.value) })} /></div>
              <div><label className="label">Неоваскуляризация</label><select className="select" value={visitForm.neovascularization} onChange={(e) => setVisitForm({ ...visitForm, neovascularization: e.target.value as Visit["neovascularization"] })}><option value="none">Нет</option><option value="moderate">Умеренная</option><option value="severe">Выраженная</option></select></div>
              <div><label className="label">Предшествующее воспаление</label><select className="select" value={visitForm.priorInflammation} onChange={(e) => setVisitForm({ ...visitForm, priorInflammation: e.target.value as Visit["priorInflammation"] })}><option value="no">Нет</option><option value="yes">Да</option></select></div>
              <div><label className="label">Отторжение в анамнезе</label><select className="select" value={visitForm.rejectionHistory} onChange={(e) => setVisitForm({ ...visitForm, rejectionHistory: e.target.value as Visit["rejectionHistory"] })}><option value="no">Нет</option><option value="yes">Да</option></select></div>
              <div style={{ gridColumn: "1 / -1" }}><label className="label">Клинические заметки</label><textarea className="textarea" value={visitForm.notes} onChange={(e) => setVisitForm({ ...visitForm, notes: e.target.value })} /></div>
            </div>

            <div style={{ display: "flex", justifyContent: "end", gap: 10, marginTop: 18 }}>
              <button className="btn outline" onClick={() => setVisitModal(false)}>Отмена</button>
              <button className="btn" onClick={saveVisit}>Сохранить визит</button>
            </div>
          </div>
        </div>
      )}
