...
import {
  Eye, UserRound, Search, Download, Plus, Trash2, LogIn, LayoutDashboard,
  BarChart3, Database, HeartPulse, X, Sparkles, Gauge, BrainCircuit, Filter,
  AlertTriangle, FileText, Microscope, ShieldCheck, Cpu, Activity
} from "lucide-react";
...
const STORAGE_KEY = "corneal_v11_vision_ml";
...
// Demo ML-inspired model: calibrated logistic score, not clinically validated.
function calcMlForecast(patient: Patient){
  ...
  const prob = Math.round(sigmoid(x) * 100);
  return {
    probability: prob,
    classLabel: prob < 25 ? "Низкая вероятность" : prob < 55 ? "Промежуточная вероятность" : "Высокая вероятность",
    calibration: "Demo logistic model",
    validated: false
  };
}
function explanation(p: Patient){
  const items:string[] = [];
  if(p.graftType==="repeat") items.push("repeat graft");
  if(p.neovascularization==="severe") items.push("severe neovascularization");
  if(p.neovascularization==="moderate") items.push("moderate neovascularization");
  if(p.priorInflammation==="yes") items.push("inflammatory history");
...
function recommendations(p: Patient){
  ...
  return r >= 65
    ? ["Усилить частоту клинического наблюдения.", "Повторить оценку воспалительных маркеров в коротком интервале.", "Оценить динамику неоваскуляризации и иммунологической активности."]
    : r >= 35
    ? ["Сократить интервал контрольного визита.", "Повторно оценить IL-6 и VEGF.", "Сопоставить лабораторные данные с клинической картиной."]
    : ["Продолжить стандартное плановое наблюдение.", "Повторно оценить маркеры по обычному протоколу.", "Сохранять контроль клинической стабильности трансплантата."];
}
...
function exportОтчёт(p?:Patient){
  if(!p) return;
  const res = calcRisk(p);
  ...
  <html><head><title>Клинический отчёт</title><style>
    ...
    <div class="grid">
      <div class="box"><strong>Patient</strong><div>${p.fullName}</div><div>ID: ${p.id}</div></div>
      <div class="box"><strong>Visit</strong><div>${p.lastVisit}</div><div>${surgeryLabel(p.surgeryType)}</div></div>
      <div class="box"><strong>Оценка риска</strong><div>${res.risk}% · ${res.level}</div></div>
      <div class="box"><strong>Прогноз вероятности отторжения</strong><div>${ml.probability}% · ${ml.classLabel}</div></div>
    </div>
    <div class="box"><strong>Biomarkers</strong><div>IL-1 ${p.il1} · IL-6 ${p.il6} · TNF-α ${p.tnf} · VEGF ${p.vegf} · TGF-β ${p.tgfb}</div></div>
    <div class="box" style="margin-top:12px"><strong>Interpretation</strong><div style="margin-top:8px">${explanation(p)}</div></div>
...
function CorneaView({ risk }:{ risk:number }) {
  const tone = risk >= 65 ? "high" : risk >= 35 ? "mid" : "low";
  return (
    <div className={`corneaStage ${tone}`}>
      <div className="corneaHalo" />
      <div className="corneaRing">
        <div className="corneaCenter">
          <div className="corneaPupil" />
        </div>
      </div>
      <div className="corneaLegend">
        <div className="tiny muted">Визуальная схема</div>
        <strong>Состояние роговицы</strong>
        <div className="small muted" style={{marginTop:6}}>
          {risk >= 65 ? "Выраженная активность факторов риска." : risk >= 35 ? "Нужен более частый контроль." : "Картина соответствует стабильному наблюдению."}
        </div>
      </div>
    </div>
  );
        ...
        <div className="grid g2">
          <div className="card">
            <div className="brand" style={{marginBottom:16}}>
              ...
              <div><div style={{fontSize:28,fontWeight:800,color:"#fff"}}>Corneal Risk Platform</div><div className="brandSub">Clinical intelligence interface</div></div>
            </div>
            <div className="grid">
          ...
          <div className="hero">
            ...
            <div className="heroText">Интерфейс для мониторинга пациентов после трансплантации роговицы с акцентом на аккуратную медицинскую подачу, автоматическую интерпретацию и демонстрацию ML-прогноза.</div>
          </div>
        </div>
      </div>
    </div>
...
export default function Page() {
  ...
  const selectedRisk = useMemo(()=>selected ? calcRisk(selected) : null,[selected]);
  const ml = useMemo(()=>selected ? calcMlForecast(selected) : null,[selected]);

  const stats = useMemo(()=>({
    total:patients.length,
    high:patients.filter(p=>calcRisk(p).risk>=65).length,
    moderate:patients.filter(p=>{const r=calcRisk(p).risk; return r>=35&&r<65;}).length,
    avg:patients.length?Math.round(patients.reduce((s,p)=>s+calcRisk(p).risk,0)/patients.length):0
  }),[patients]);
  ...
  const filteredPatients = useMemo(()=>{
    ...
    arr.sort((a,b)=>{
      const ra=calcRisk(a).risk, rb=calcRisk(b).risk;
  ...
  const save = ()=>{
    ...
    const n:Patient = {...form, age:Number(form.age)||0, il1:Number(form.il1)||0, il6:Number(form.il6)||0, tnf:Number(form.tnf)||0, vegf:Number(form.vegf)||0, tgfb:Number(form.tgfb)||0};
    const exists=patients.some(p=>p.id===form.id);
    setPatients(prev=>exists?prev.map(p=>p.id===form.id?n:p):[n,...prev]);
  ...
  return (
    <div className="page">
      <aside className="sidebar">
        ...
        <div className="userBox"><div style={{fontWeight:700,color:"#fff"}}>Пользователь</div><div style={{marginTop:6}}>{user.email}</div><div className="tiny" style={{marginTop:12}}>Оценка риска и ML-прогноз в этой демонстрационной версии не являются клинически валидированными моделями.</div></div>
      </aside>
      ...
      <main className="main">
        <div className="container">
          <div className="hero">
            ...
            <div className="heroText">Аккуратная цифровая платформа: реестр пациентов, ориентировочная стратификация риска, визуальная схема роговицы, автоматическая интерпретация показателей и демонстрационный ML-прогноз вероятности отторжения.</div>
            <div className="heroActions">
              <button className="btn outline" onClick={openNew}><Plus size={16}/>Новый пациент</button>
              <button className="btn outline" onClick={()=>exportОтчёт(selected)}><Download size={16}/>PDF-отчёт</button>
            </div>
            <div className="grid g4" style={{marginTop:22}}>
              <div className="statMini"><span className="muted tiny">Risk engine</span><strong>Live</strong><span className="tiny muted">Оценка обновляется по данным пациента</span></div>
              <div className="statMini"><span className="muted tiny">Clinical markers</span><strong>5</strong><span className="tiny muted">IL-1, IL-6, TNF-α, VEGF, TGF-β</span></div>
              <div className="statMini"><span className="muted tiny">Operation types</span><strong>4</strong><span className="tiny muted">PKP, DALK, DSAEK, DMEK</span></div>
              <div className="statMini"><span className="muted tiny">ML forecast</span><strong>{ml?.probability ?? 0}%</strong><span className="tiny muted">Demo rejection probability</span></div>
            </div>
          </div>
            ...
            <div className="grid g3">
              <div className="card" style={{gridColumn:"span 2"}}>
                <div className="sectionTitle">
                  <div>
                    ...
                    <div className="muted small">Ключевые клинические параметры, ориентировочная оценка риска, прогноз вероятности отторжения и интерпретация в одной панели</div>
                  </div>
                  <div className="kpiPill"><ShieldCheck size={14}/> Демонстрационный прогноз</div>
                </div>
                ...
                <div className="grid g2" style={{marginTop:18}}>
                  <div className="grid">
                    <div className="panel">
                      <div style={{display:"flex",justifyContent:"space-between",gap:12}}>
                        <div>
                          ...
                          <div style={{fontSize:26,fontWeight:850,marginTop:6}}>{selected.fullName}</div>
                    ...
                    <div className={`alertBox ${alertClass}`}>
                      <div style={{display:"flex",alignItems:"center",gap:10,fontWeight:800}}><AlertTriangle size={18}/> Клиническое предупреждение</div>
                      <div className="small" style={{marginTop:8}}>{selectedRisk.risk >= 65 ? "Высокая ориентировочная вероятность иммунологического осложнения. Рекомендуется усиленный мониторинг." : selectedRisk.risk >= 35 ? "Есть факторы, требующие более частого наблюдения." : "Текущая картина соответствует стабильному наблюдению."}</div>
                    </div>

                    <div className="softCard glowBlue">
                      <div className="headerRow">
                        <div>
                          <div className="muted tiny">Прогноз вероятности отторжения</div>
                          <div style={{fontSize:34,fontWeight:900,marginTop:4}}>{ml.probability}%</div>
                          <div className="small muted">{ml.classLabel}</div>
                        </div>
                        <div className="iconWrap"><Cpu size={18}/></div>
                      </div>
                      <div className="progress" style={{marginTop:14}}><div className="bar" style={{width:`${ml.probability}%`}}/></div>
                      <div className="note" style={{marginTop:12}}>Это демонстрационная логистическая модель без клинической валидации. Она показывает, как может выглядеть ML-блок внутри продукта.</div>
                    </div>
                    ...
                    <div className="card">
                      ...
                      <div className="timelineList">
                        ...
                        ].map((it,idx)=><div key={idx} className="timelineItem"><div className="timelineDate">{it.date}</div><div><div className="timelineTitle">{it.title}</div><div className="small muted" style={{marginTop:4}}>{it.text}</div></div></div>)}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid">
                    <div className="panel premiumTopGrid">
                      <div className="premiumGaugeBox">
                        ...
                        <div className="small muted" style={{marginTop:2}}>Оценка риска</div>
                        <div className="progress" style={{marginTop:8}}><div className="bar" style={{width:`${selectedRisk.risk}%`}}/></div>
                      </div>
                      <CorneaView risk={selectedRisk.risk} />
                    </div>

                    <div className="markerGrid">
                      {[["IL-1", selected.il1],["IL-6", selected.il6],["TNF-α", selected.tnf],["VEGF", selected.vegf],["TGF-β", selected.tgfb]].map(([label, value]) =>
                        <div key={String(label)} className="marker"><span className="markerTitle">{label}</span><div className="markerValue">{value}</div></div>
                      )}
                    </div>

                    <div className="grid g2">
                      <div className="card">
                        <div className="muted small">Факторы риска</div>
                        <div className="factorList">
                          {[
                            ...(selected.graftType==="repeat"?[["Повторная трансплантация",18] as const]:[]),
                            ...(selected.neovascularization==="severe"?[["Выраженная неоваскуляризация",24] as const]:selected.neovascularization==="moderate"?[["Умеренная неоваскуляризация",14] as const]:[]),
                            ...(selected.priorInflammation==="yes"?[["Воспалительный анамнез",12] as const]:[]),
                            ...(selected.rejectionHistory==="yes"?[["Отторжение в анамнезе",18] as const]:[]),
                            ["Повышение IL-1",Math.round(clamp((Number(selected.il1)||0)/2,0,10))] as const,
                            ["Повышение IL-6",Math.round(clamp((Number(selected.il6)||0)/2,0,12))] as const,
                            ["Повышение VEGF",Math.round(clamp((Number(selected.vegf)||0)/10,0,12))] as const
                          ].filter(x=>x[1]>0).slice(0,5).map((c,i)=><div key={i} className="factorRow"><span>{c[0]}</span><span className="factorScore">+{c[1]}</span></div>)}
                        </div>
                      </div>
                      ...
                      <div className="card">
                        ...
                        <div className="heatmapGrid">
                          {heatItems.map(([name,value])=>{
                            ...
                            return <div key={name} className="heatItem"><div className="heatHeader"><span>{name}</span><span className={`statusDot status-${s}`}></span></div><div className="heatValue">{value}</div><div className="tiny muted">{s==="high"?"Высокий":s==="mid"?"Промежуточный":"Низкий"}</div></div>;
                          })}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="card">
                      <div className="muted small">Автоматическая интерпретация риска</div>
              ...
              <div className="card">
                ...
                <div className="grid" style={{marginTop:18}}>
                  ...
                  <button className="btn outline" onClick={()=>exportОтчёт(selected)}><Download size={16}/>Скачать PDF-отчёт</button>
                ...
                <div className="softCard glowGreen" style={{marginTop:16}}>
                  <div className="headerRow">
                    <div>
                ...
                <div className="softCard glowBlue" style={{marginTop:16}}>
                  ...
                  <div className="small muted" style={{marginTop:8}}>В этой версии показаны демонстрационные блоки прогнозирования. Для реальной модели понадобится клинический набор данных и отдельная валидация.</div>
                </div>
              </div>
            </div>
          </>}
          ...
          {page==="patients" && selected && selectedRisk && ml && (
            <div className="grid g3">
              <div className="card">
                ...
                <div className="muted small">Быстрый доступ к пациентам, сортировка по риску и клиническим заметкам</div>
                    ...
                    return (
                      <button key={patient.id} className={`patientBtn ${selectedId===patient.id ? "active" : ""}`} onClick={()=>setSelectedId(patient.id)}>
                        <div style={{display:"flex",justifyContent:"space-between",gap:10}}>
                          <div><div style={{fontWeight:700}}>{patient.fullName}</div><div className="muted small">{patient.id} · {patient.surgeryType} · визит {patient.lastVisit}</div></div>
              ...
              <div className="card" style={{gridColumn:"span 2"}}>
                <div className="headerRow">
                  <div><h2>Карточка пациента</h2><div className="muted small">Структурированные сведения, биомаркеры и интерпретация ориентировочного риска</div></div>
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button className="btn outline" onClick={()=>{setForm(selected);setModal(true);}}>Редактировать</button><button className="btn danger" onClick={()=>remove(selected.id)}><Trash2 size={16}/>Удалить</button></div>
                </div>

                <div className="tabs">
                  <button className={`tab ${tab==="summary" ? "active" : ""}`} onClick={()=>setTab("summary")}>Сводка</button>
                  <button className={`tab ${tab==="markers" ? "active" : ""}`} onClick={()=>setTab("markers")}>Маркеры</button>
                ...
                {tab==="summary" && <div className="grid g3" style={{marginTop:18}}>
                  ...
                  <div className="card" style={{gridColumn:"1 / -1"}}><div className="muted small">Автоматическая клиническая сводка</div><div style={{marginTop:8}}>{explanation(selected)}</div></div>
                </div>}

                {tab==="markers" && <div className="grid g2" style={{marginTop:18}}>
                  <div className="card"><div className="muted small">Карта биомаркеров</div><div className="heatmapGrid" style={{marginTop:12}}>{heatItems.map(([name,value])=>{ const s=heatStatus(name,value); return <div key={name} className="heatItem"><div className="heatHeader"><span>{name}</span><span className={`statusDot status-${s}`}></span></div><div className="heatValue">{value}</div><div className="tiny muted">{s==="high"?"Высокий":s==="mid"?"Промежуточный":"Низкий"}</div></div>; })}</div></div>
                  <div className="card"><div className="muted small">Схема роговицы</div><div style={{marginTop:12}}><CorneaView risk={selectedRisk.risk} /></div></div>
                </div>}
                ...
                {tab==="recommendations" && <div className="grid g2" style={{marginTop:18}}>
                  <div className="grid">
                    ...
                    <div className="softCard glowBlue">
                      <div className="headerRow"><div><div className="muted tiny">Прогноз вероятности отторжения</div><strong>{ml.probability}% · {ml.classLabel}</strong></div><Cpu size={18}/></div>
                      <div className="small muted" style={{marginTop:8}}>Демонстрационная логистическая модель без клинической валидации.</div>
                    </div>
                  </div>
                  <div className="grid">
                    <div className="card"><div className="muted small">Автоматическая интерпретация риска</div><div style={{marginTop:8}}>{explanation(selected)}</div></div>
                ...
                {tab==="report" && <div className="reportSheet" style={{marginTop:18}}>
                  ...
                  <div className="reportMeta">
                    ...
                    <div className="reportBlock"><strong>Прогноз вероятности отторжения</strong><div>{ml.probability}% · {ml.classLabel}</div></div>
                  </div>
                  <div className="reportBlock" style={{marginTop:14}}><strong>Клиническая интерпретация</strong><div style={{marginTop:8}}>{explanation(selected)}</div></div>
                  <div style={{marginTop:16}}><button className="btn" onClick={()=>exportОтчёт(selected)}><Download size={16}/>Печать / Сохранить PDF</button></div>
                </div>}
              </div>
            </div>
          )}
            ...
            <div className="grid g2">
              <div className="card">
                ...
                <div className="listCompact" style={{marginTop:14}}>
                  <div className="softCard"><div className="headerRow"><span>Средний риск по базе</span><strong>{stats.avg}%</strong></div></div>
                  <div className="softCard"><div className="headerRow"><span>Высокий риск</span><strong>{stats.high}</strong></div></div>
                  <div className="softCard"><div className="headerRow"><span>Умеренный риск</span><strong>{stats.moderate}</strong></div></div>
                </div>
              </div>
              <div className="card">
                <h2>Блоки AI и ML</h2>
                <div className="note" style={{marginTop:14}}>AI explanation в этой версии основан на правилах интерпретации признаков, а Прогноз вероятности отторжения — на демонстрационной логистической модели. Для реального медицинского использования потребуется обучающий клинический датасет, разделение на обучающую, валидационную и тестовую выборки и внешняя валидация.</div>
              </div>
            </div>
          )}
            ...
            <div className="grid g2">
              <div className="card">
                ...
                <div className="grid small muted" style={{marginTop:18}}>
                  <div>Платформа помогает структурировать наблюдение, визуализировать совокупность клинических факторов и быстро получать целостную картину по пациенту после трансплантации роговицы.</div>
                  <div>Показатель риска является ориентировочным и предназначен для демонстрации логики стратификации. Он не должен использоваться как автономное клиническое решение без оценки врача.</div>
                  <div>Входные параметры: тип трансплантации, вид операции, неоваскуляризация, воспаление в анамнезе, предыдущие эпизоды отторжения, IL-1, IL-6, TNF-α, VEGF и TGF-β.</div>
                </div>
              </div>
              <div className="card">
                <h2>Как превратить demo в реальную ML-модель</h2>
                <div className="grid small muted" style={{marginTop:18}}>
                  <div>1. Собрать размеченный клинический датасет с реальными исходами отторжения.</div>
                  <div>2. Разделить данные на train / validation / test.</div>
                  <div>3. Обучить и откалибровать модель вероятности.</div>
                  <div>4. Проверить чувствительность, специфичность, ROC-AUC и calibration.</div>
                  <div>5. Выполнить внешнюю валидацию на независимой когорте.</div>
                </div>
              </div>
            </div>
          )}
          ...
          {page==="about" && (
            <div className="grid g2">
              <div className="card">
                <h2>О платформе</h2>
                <div className="grid small muted" style={{marginTop:18}}>
                  <div>Corneal Risk Platform объединяет клинические данные пациента, ориентировочную стратификацию риска, интерпретацию биомаркеров и аналитические панели в единой рабочей среде.</div>
                  <div>Интерфейс подходит для демонстрации концепции цифровой поддержки принятия решений в офтальмологии и для презентации потенциального продукта медицинского класса.</div>
                </div>
              </div>
              <div className="card">
                <h2>Сильные стороны текущей версии</h2>
                <div className="grid small muted" style={{marginTop:18}}>
                  <div>• Единая центральная клиническая карточка</div>
                  <div>• Схема роговицы</div>
                  <div>• Heatmap биомаркеров</div>
                  <div>• Автоматическая интерпретация риска</div>
                  <div>• Demo Прогноз вероятности отторжения</div>
                  <div>• Printable PDF-отчёт</div>
                </div>
              </div>
            </div>
          )}
      ...
      {modal && <div className="modalBg"><div className="modal">
        <div className="headerRow"><div><h2 style={{margin:0,color:"#fff"}}>{patients.some(p=>p.id===form.id) ? "Редактирование пациента" : "Новый пациент"}</h2><div className="muted small">Заполните клинические параметры для ориентировочной оценки риска.</div></div><button className="btn outline" onClick={()=>setModal(false)}><X size={16}/></button></div>
