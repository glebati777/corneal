...
const STORAGE_KEY = "corneal_v9_ultimate_fixed";
...
const trendData = [
  { date: "Нед 1", IL1: 6, IL6: 8, VEGF: 46, risk: 42 },
  { date: "Нед 2", IL1: 8, IL6: 11, VEGF: 55, risk: 56 },
  { date: "Нед 3", IL1: 10, IL6: 13, VEGF: 60, risk: 72 },
  { date: "Нед 4", IL1: 7, IL6: 9, VEGF: 49, risk: 60 }
];
...
function calculateRisk(p: Patient){ let s=0; if(p.graftType==="repeat") s+=18; if(p.neovascularization==="moderate") s+=14; if(p.neovascularization==="severe") s+=24; if(p.priorInflammation==="yes") s+=12; if(p.rejectionHistory==="yes") s+=18; s+=clamp((Number(p.il1)||0)/2,0,10)+clamp((Number(p.il6)||0)/2,0,12)+clamp((Number(p.tnf)||0)/2,0,10)+clamp((Number(p.vegf)||0)/10,0,12)+clamp((Number(p.tgfb)||0)/12,0,10); const risk=clamp(Math.round(s),0,100); return { risk, level:risk<35?"Низкий":risk<65?"Умеренный":"Высокий", status:risk<35?"Стабильное наблюдение":risk<65?"Усиленный мониторинг":"Повышенное внимание", recommendation:risk<35?"Рекомендуется стандартное наблюдение по принятому графику.":risk<65?"Желателен более частый контроль клинической картины и биомаркеров.":"Нужен более плотный мониторинг и обсуждение тактики ведения на клиническом уровне." }; }
function contributors(p: Patient){ const arr:[string,number][]=[]; if(p.graftType==="repeat") arr.push(["Repeat graft",18]); if(p.neovascularization==="moderate") arr.push(["Moderate neovascularization",14]); if(p.neovascularization==="severe") arr.push(["Severe neovascularization",24]); if(p.priorInflammation==="yes") arr.push(["Inflammatory history",12]); if(p.rejectionHistory==="yes") arr.push(["Rejection history",18]); arr.push(["IL-1 elevation",Math.round(clamp((Number(p.il1)||0)/2,0,10))],["IL-6 elevation",Math.round(clamp((Number(p.il6)||0)/2,0,12))],["VEGF elevation",Math.round(clamp((Number(p.vegf)||0)/10,0,12))]); return arr.filter((x)=>x[1]>0).sort((a,b)=>b[1]-a[1]).slice(0,5); }
function aiExplanation(p: Patient){ const c = contributors(p).map(x=>x[0].toLowerCase()).join(", "); return `${c ? "Основной вклад в риск дают: " + c + "." : "Выраженных факторов повышения риска не выявлено."} Совокупность параметров формирует ${calculateRisk(p).level.toLowerCase()} профиль риска и требует интерпретации в контексте клинической картины.`; }
function smartRecommendations(p: Patient){ const r=calculateRisk(p).risk; return r>=65?["Усилить частоту клинического наблюдения.","Повторить оценку воспалительных маркеров в коротком интервале.","Оценить динамику неоваскуляризации."]:r>=35?["Сократить интервал контрольного визита.","Повторно оценить IL-6 и VEGF.","Сопоставить лабораторные данные с клинической картиной."]:["Продолжить стандартное плановое наблюдение.","Повторно оценить маркеры по обычному протоколу.","Сохранять контроль клинической стабильности трансплантата."]; }
...
function exportReport(p?:Patient){ if(!p) return; const res = calculateRisk(p); const html = `<html><head><title>Clinical Report</title><style>body{font-family:Arial,sans-serif;padding:40px;color:#111}h1{margin:0 0 12px 0}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:20px 0}.box{border:1px solid #d1d5db;border-radius:12px;padding:14px}</style></head><body><h1>Clinical Report</h1><div>Structured follow-up summary after corneal transplantation</div><div class="grid"><div class="box"><strong>Patient</strong><div>${p.fullName}</div><div>ID: ${p.id}</div></div><div class="box"><strong>Visit</strong><div>${p.lastVisit}</div><div>${surgeryLabel(p.surgeryType)}</div></div><div class="box"><strong>Risk score</strong><div>${res.risk}% · ${res.level}</div></div><div class="box"><strong>Markers</strong><div>IL-1 ${p.il1} · IL-6 ${p.il6} · TNF-α ${p.tnf} · VEGF ${p.vegf}</div></div></div><div class="box"><strong>Clinical summary</strong><div style="margin-top:8px">${aiExplanation(p)}</div></div><script>window.onload=()=>window.print()</script></body></html>`; const w = window.open("", "_blank"); if(!w) return; w.document.write(html); w.document.close(); }
function RiskGauge({ value, label }:{ value:number; label:string }){ const angle=`${-90 + (value/100)*180}deg`; const gaugeClass=value>=65?"high":value>=35?"mid":"low"; return <div className="gaugeWrap"><div className={`gaugeShell ${gaugeClass}`}><div className="gaugeArc"/><div className="gaugeInner"/><div className="gaugeNeedle" style={{["--angle" as any]:angle}}/><div className="gaugeDot"/><div className="gaugeCenterText"><div className="gaugeValue">{value}%</div><div className="gaugeLabel">{label}</div></div><div className="gaugeLabels"><span>LOW</span><span>MODERATE</span><span>HIGH</span></div></div></div>; }
function StatCard({ title, value, caption, icon:Icon }:{ title:string; value:string|number; caption:string; icon:any }){ return <div className="statCard"><div className="statTop"><div><div className="muted small">{title}</div><div className="statValue">{value}</div><div className="muted small">{caption}</div></div><div className="iconWrap"><Icon size={20} /></div></div></div>; }
...
export default function Page(){
  ...
  const stats = useMemo(()=>({ total:patients.length, high:patients.filter(p=>calculateRisk(p).risk>=65).length, moderate:patients.filter(p=>{const r=calculateRisk(p).risk; return r>=35&&r<65;}).length, avg:patients.length?Math.round(patients.reduce((s,p)=>s+calculateRisk(p).risk,0)/patients.length):0 }),[patients]);

  const riskChart = [{name:"Низкий",value:patients.filter(p=>calculateRisk(p).risk<35).length},{name:"Умеренный",value:patients.filter(p=>{const r=calculateRisk(p).risk; return r>=35&&r<65;}).length},{name:"Высокий",value:patients.filter(p=>calculateRisk(p).risk>=65).length}];
  const biomarkerChart = [{marker:"IL-1",value:Math.round((patients.reduce((a,p)=>a+Number(p.il1||0),0)/patients.length)*10)/10},{marker:"IL-6",value:Math.round((patients.reduce((a,p)=>a+Number(p.il6||0),0)/patients.length)*10)/10},{marker:"TNF-α",value:Math.round((patients.reduce((a,p)=>a+Number(p.tnf||0),0)/patients.length)*10)/10},{marker:"VEGF",value:Math.round((patients.reduce((a,p)=>a+Number(p.vegf||0),0)/patients.length)*10)/10},{marker:"TGF-β",value:Math.round((patients.reduce((a,p)=>a+Number(p.tgfb||0),0)/patients.length)*10)/10}];
  ...
  return (
    <div className="page">
      ...
      <main className="main"><div className="container">
        ...
        {page==="dashboard" && selected && selectedRisk && <div className="grid">
            ...
            <div className="card" style={{gridColumn:"span 2"}}>
              ...
              <div className="grid g2" style={{marginTop:18}}>
                <div className="grid">
                  ...
                  <div className="card"><div className="muted small">Patient Timeline</div><div className="timelineList">{buildTimeline(selected).map((it,idx)=><div key={idx} className="timelineItem"><div className="timelineDate">{it.date}</div><div><div className="timelineTitle">{it.title}</div><div className="small muted" style={{marginTop:4}}>{it.text}</div></div></div>)}</div></div>
                  <div className="card"><div className="muted small">AI-assisted Risk Explanation</div><div style={{marginTop:8,color:"#fff"}}>{aiExplanation(selected)}</div></div>
                </div>

                <div className="grid">
                  <div className="panel"><div className="grid g2" style={{alignItems:"center"}}><RiskGauge value={selectedRisk.risk} label={selectedRisk.level} /><div className="grid"><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><span className="badge dark">{selectedRisk.level} риск</span><span className="badge outlineBadge">{selectedRisk.status}</span></div><div className="small muted">Risk score</div><div className="progress"><div className="bar" style={{width:`${selectedRisk.risk}%`}}/></div><div className="card" style={{padding:14, marginTop:8}}><div className="muted tiny">Biomarker Trend</div><div className="miniTrend">{getMiniTrend(selected).map((t,i)=><div key={i} className="miniItem"><span>{t.name}</span><strong>{t.trend}</strong></div>)}</div></div></div></div></div>
                  ...
                  <div className="card"><div className="muted small">Risk Contributors</div><div className="factorList">{contributors(selected).map((c, i)=><div key={i} className="factorRow"><span>{c[0]}</span><span className="factorScore">+{c[1]}</span></div>)}</div></div>
        ...
        {page==="patients" && selected && selectedRisk && <div className="grid g3">
          ...
          <div className="card" style={{gridColumn:"span 2"}}>
            ...
            {tab==="summary" && <div className="grid g3" style={{marginTop:18}}>
              ...
              <div className="card" style={{gridColumn:"1 / -1"}}><div className="muted small">Автоматический clinical summary</div><div style={{marginTop:8,color:"#fff"}}>{buildClinicalSummary(selected)}</div></div>
            </div>}

            {tab==="markers" && <div className="grid" style={{marginTop:18}}>
              <div className="markerGrid">{[["IL-1", selected.il1],["IL-6", selected.il6],["TNF-α", selected.tnf],["VEGF", selected.vegf],["TGF-β", selected.tgfb]].map(([label,value])=><div key={String(label)} className="marker"><span className="markerTitle">{label}</span><div className="markerValue">{value}</div></div>)}</div>
              <div className="grid g2">
                <div className="card"><div className="muted small" style={{marginBottom:10}}>Динамика маркеров по визитам</div><div style={{height:300}}><ResponsiveContainer width="100%" height="100%"><LineChart data={trendData}><CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,.18)" /><XAxis dataKey="date" stroke="#94a3b8" /><YAxis stroke="#94a3b8" /><Tooltip /><Legend /><Line type="monotone" dataKey="IL1" name="IL-1" strokeWidth={2} /><Line type="monotone" dataKey="IL6" name="IL-6" strokeWidth={2} /><Line type="monotone" dataKey="VEGF" name="VEGF" strokeWidth={2} /></LineChart></ResponsiveContainer></div></div>
                <div className="card"><div className="muted small" style={{marginBottom:10}}>Risk Trajectory</div><div style={{height:300}}><ResponsiveContainer width="100%" height="100%"><LineChart data={trendData}><CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,.18)" /><XAxis dataKey="date" stroke="#94a3b8" /><YAxis stroke="#94a3b8" /><Tooltip /><Line type="monotone" dataKey="risk" name="Risk %" strokeWidth={3} /></LineChart></ResponsiveContainer></div></div>
              </div>
              <div className="card"><div className="muted small">Biomarker Heatmap</div><div className="heatmapGrid">{heatItems.map(([name,value])=>{ const s=heatStatus(name,value); return <div key={name} className="heatItem"><div className="heatHeader"><span>{name}</span><span className={`statusDot status-${s}`}></span></div><div className="heatValue">{value}</div><div className="tiny muted">{s==="high"?"High":s==="mid"?"Intermediate":"Low"}</div></div>; })}</div></div>
            </div>}
            ...
            {tab==="recommendations" && <div className="grid g2" style={{marginTop:18}}>
              ...
              <div className="grid">
                <div className="card"><div className="muted small">Risk Contributors</div><div className="factorList">{contributors(selected).map((c, i)=><div key={i} className="factorRow"><span>{c[0]}</span><span className="factorScore">+{c[1]}</span></div>)}</div></div>
                <div className="card"><div className="muted small">AI-assisted Risk Explanation</div><div style={{marginTop:8,color:"#fff"}}>{aiExplanation(selected)}</div></div>
                <div className="card"><div className="muted small">Smart Recommendations</div><div className="recoList">{smartRecommendations(selected).map((r,i)=><div key={i} className="recoItem"><span>{r}</span><strong>•</strong></div>)}</div></div>
              </div>
            </div>}
            ...
            {tab==="report" && <div className="reportSheet" style={{marginTop:18}}>
              ...
              <div className="reportBlock" style={{marginTop:14}}><strong>Clinical summary</strong><div style={{marginTop:8}}>{buildClinicalSummary(selected)}</div></div>
        ...
        {page==="methodology" && <div className="grid g3">
          ...
          <div className="card"><h2>Новые улучшения v9</h2><div className="grid" style={{marginTop:18,gap:10}}><div className="factorRow"><span>AI-assisted explanation</span><strong>New</strong></div><div className="factorRow"><span>Risk trajectory</span><strong>New</strong></div><div className="factorRow"><span>Smart recommendations</span><strong>New</strong></div></div></div>
        </div>}
