...
function explanation(p: Patient){ const items:string[]=[]; if(p.graftType==="repeat") items.push("repeat graft"); if(p.neovascularization==="severe") items.push("severe neovascularization"); if(p.neovascularization==="moderate") items.push("moderate neovascularization"); if(p.priorInflammation==="yes") items.push("inflammatory history"); if(Number(p.il6)>=10) items.push("elevated IL-6"); if(Number(p.vegf)>=60) items.push("elevated VEGF"); return `${items.length ? "Наибольший вклад в ориентировочный риск дают: " + items.join(", ") + "." : "Выраженных факторов повышения риска не выявлено."} Совокупность параметров формирует ${calc(p).level.toLowerCase()} профиль риска.`; }
    ...
    <div className="page">
      ...
      <main className="main"><div className="container">
        <div className="hero">
          ...
          <div className="heroActions"><button className="btn outline" onClick={openNew}><Plus size={16}/>Новый пациент</button><button className="btn outline" onClick={()=>exportReport(selected)}><Download size={16}/>PDF report</button></div><div className="grid g4" style={{marginTop:22}}><div className="statMini"><span className="muted tiny">Risk engine</span><strong>Live</strong><span className="tiny muted">Оценка обновляется по данным пациента</span></div><div className="statMini"><span className="muted tiny">Clinical markers</span><strong>5</strong><span className="tiny muted">IL-1, IL-6, TNF-α, VEGF, TGF-β</span></div><div className="statMini"><span className="muted tiny">Operation types</span><strong>4</strong><span className="tiny muted">PKP, DALK, DSAEK, DMEK</span></div><div className="statMini"><span className="muted tiny">Report mode</span><strong>PDF</strong><span className="tiny muted">Печать и сохранение clinical summary</span></div></div>
        </div>
        ...
        {page==="dashboard" && selected && selectedRisk && <>
          ...
          <div className="grid g3">
            <div className="card" style={{gridColumn:"span 2"}}>
              ...
              <div className="muted small">Ключевые клинические параметры, ориентировочная оценка риска и интерпретация в одной панели</div>
        ...
        {page==="patients" && selected && selectedRisk && <div className="grid g3">
          ...
          <div className="card" style={{gridColumn:"span 2"}}>
            <div style={{display:"flex",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}><div><h2>Карточка пациента</h2><div className="muted small">Структурированные сведения, биомаркеры и интерпретация ориентировочного риска</div></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button className="btn outline" onClick={()=>{setForm(selected);setModal(true);}}>Редактировать</button><button className="btn danger" onClick={()=>remove(selected.id)}><Trash2 size={16}/>Удалить</button></div></div>
            <div className="tabs">
        ...
        {page==="methodology" && <div className="grid g2">
          <div className="card"><h2>Методологические принципы</h2><div className="grid small muted" style={{marginTop:18}}><div>Платформа помогает структурировать наблюдение, визуализировать совокупность клинических факторов и быстро получать целостную картину по пациенту после трансплантации роговицы.</div><div>Показатель риска является ориентировочным и предназначен для демонстрации логики стратификации. Он не должен использоваться как автономное клиническое решение без оценки врача.</div><div>Входные параметры: тип трансплантации, вид операции, неоваскуляризация, воспаление в анамнезе, предыдущие эпизоды отторжения, IL-1, IL-6, TNF-α, VEGF и TGF-β.</div></div></div>
        ...
        {page==="about" && <div className="grid g2">
          <div className="card"><h2>О платформе</h2><div className="grid small muted" style={{marginTop:18}}><div>Corneal Risk Platform объединяет клинические данные пациента, ориентировочную стратификацию риска, интерпретацию биомаркеров и аналитические панели в единой рабочей среде.</div><div>Интерфейс подходит для демонстрации концепции цифровой поддержки принятия решений в офтальмологии и для презентации потенциального продукта медицинского класса.</div></div></div>
          <div className="card"><h2>Сильные стороны текущей версии</h2><div className="grid small muted" style={{marginTop:18}}><div>• Единая central clinical card</div><div>• Heatmap биомаркеров</div><div>• AI-assisted explanation</div><div>• Printable PDF report</div></div></div>
