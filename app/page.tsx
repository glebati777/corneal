...
import {
  ...
  HeartPulse, X, Sparkles, Gauge, BrainCircuit, Filter, WandSparkles, Activity, AlertTriangle
} from "lucide-react";
import {
...
type Patient = {
  id: string; fullName: string; age: number | string; sex: string;
  graftType: string; surgeryType: string; neovascularization: string;
  priorInflammation: string; rejectionHistory: string;
  il1: number | string; il6: number | string; tnf: number | string; vegf: number | string; tgfb: number | string;
  notes: string; lastVisit: string;
};

const STORAGE_KEY = "corneal_v8_elite";
...
const trendData = [
  { date: "Нед 1", IL1: 6, IL6: 8, TNFa: 5, VEGF: 46, TGFB: 32, risk: 42 },
  { date: "Нед 2", IL1: 8, IL6: 11, TNFa: 7, VEGF: 55, TGFB: 34, risk: 56 },
  { date: "Нед 3", IL1: 10, IL6: 13, TNFa: 8, VEGF: 60, TGFB: 38, risk: 72 },
  { date: "Нед 4", IL1: 7, IL6: 9, TNFa: 6, VEGF: 49, TGFB: 33, risk: 60 }
];
...
function calculateRisk(patient: Patient){
  ...
  return {
    ...
    recommendation: risk < 35 ? "Рекомендуется стандартное наблюдение по принятому графику." : risk < 65 ? "Желателен более частый контроль клинической картины и биомаркеров." : "Нужен более плотный мониторинг и обсуждение тактики ведения на клиническом уровне."
  };
}
function buildClinicalSummary(patient: Patient){
  ...
  parts.push(patient.neovascularization==="severe" ? "Отмечается выраженная неоваскуляризация." : patient.neovascularization==="moderate" ? "Отмечается умеренная неоваскуляризация." : "Клинически значимой неоваскуляризации не отмечено.");
...
function buildTimeline(patient: Patient){
  return [
    { date: patient.lastVisit, title: "Последний визит", text: `Контроль после операции. Зафиксированы маркеры: IL-1 ${patient.il1}, IL-6 ${patient.il6}, TNF-α ${patient.tnf}.` },
    { date: "2026-02-22", title: "Оценка воспалительной активности", text: patient.priorInflammation==="yes" ? "Сохраняются данные за воспалительный компонент." : "Значимого воспалительного компонента не отмечено." },
    { date: "2026-01-10", title: "Операция", text: surgeryLabel(patient.surgeryType) }
  ];
}
function getRiskContributors(patient: Patient){
  const items:{label:string; score:number}[] = [];
  if(patient.graftType==="repeat") items.push({label:"Repeat graft", score:18});
  if(patient.neovascularization==="moderate") items.push({label:"Moderate neovascularization", score:14});
  if(patient.neovascularization==="severe") items.push({label:"Severe neovascularization", score:24});
  if(patient.priorInflammation==="yes") items.push({label:"Inflammatory history", score:12});
  if(patient.rejectionHistory==="yes") items.push({label:"Rejection history", score:18});
  const il1 = clamp((Number(patient.il1)||0)/2,0,10); if(il1>0) items.push({label:"IL-1 elevation", score:Math.round(il1)});
  const il6 = clamp((Number(patient.il6)||0)/2,0,12); if(il6>0) items.push({label:"IL-6 elevation", score:Math.round(il6)});
  const vegf = clamp((Number(patient.vegf)||0)/10,0,12); if(vegf>0) items.push({label:"VEGF elevation", score:Math.round(vegf)});
  return items.sort((a,b)=>b.score-a.score).slice(0,5);
}
function getMiniTrend(patient: Patient){
  const il1 = Number(patient.il1)||0;
  const il6 = Number(patient.il6)||0;
  const vegf = Number(patient.vegf)||0;
  return [
    { name:"IL-1", trend: il1 >= 8 ? "↑" : il1 >= 4 ? "→" : "↓" },
    { name:"IL-6", trend: il6 >= 10 ? "↑" : il6 >= 5 ? "→" : "↓" },
    { name:"VEGF", trend: vegf >= 60 ? "↑" : vegf >= 30 ? "→" : "↓" }
  ];
}
function exportReport(patient?: Patient){
  ...
  const html = `
  <html><head><title>Clinical Report</title><style>
  body{font-family:Arial,sans-serif;padding:40px;color:#111}
  h1{margin:0 0 12px 0}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:20px 0}
  .box{border:1px solid #d1d5db;border-radius:12px;padding:14px}.muted{color:#555;font-size:14px}
  </style></head><body>
  <h1>Clinical Report</h1>
  <div class="muted">Structured follow-up summary after corneal transplantation</div>
  <div class="grid">
    <div class="box"><strong>Patient</strong><div>${patient.fullName}</div><div>ID: ${patient.id}</div></div>
    <div class="box"><strong>Visit</strong><div>${patient.lastVisit}</div><div>${surgeryLabel(patient.surgeryType)}</div></div>
    <div class="box"><strong>Risk score</strong><div>${result.risk}% · ${result.level}</div></div>
    <div class="box"><strong>Markers</strong><div>IL-1 ${patient.il1} · IL-6 ${patient.il6} · TNF-α ${patient.tnf} · VEGF ${patient.vegf}</div></div>
  </div>
  <div class="box"><strong>Clinical summary</strong><div style="margin-top:8px">${summary}</div></div>
...
export default function Page(){
  ...
  const selectedTimeline = useMemo(()=>selected ? buildTimeline(selected) : [],[selected]);
  const selectedContributors = useMemo(()=>selected ? getRiskContributors(selected) : [],[selected]);
  const selectedTrend = useMemo(()=>selected ? getMiniTrend(selected) : [],[selected]);

  const filteredPatients = useMemo(()=>{
    let arr = [...patients].filter(p=>[p.id,p.fullName,p.notes].some(v=>String(v).toLowerCase().includes(query.toLowerCase())));
    if(riskFilter !== "all"){ arr = arr.filter(p=>{ const r=calculateRisk(p).risk; if(riskFilter==="low") return r<35; if(riskFilter==="mid") return r>=35&&r<65; return r>=65; }); }
    arr.sort((a,b)=>{ const ra=calculateRisk(a).risk, rb=calculateRisk(b).risk; if(sortBy==="risk-desc") return rb-ra; if(sortBy==="risk-asc") return ra-rb; if(sortBy==="name") return a.fullName.localeCompare(b.fullName); return String(b.lastVisit).localeCompare(String(a.lastVisit));});
    return arr;
  ...
  const save = ()=>{
    ...
    const normalized: Patient = { ...form, age:Number(form.age)||0, il1:Number(form.il1)||0, il6:Number(form.il6)||0, tnf:Number(form.tnf)||0, vegf:Number(form.vegf)||0, tgfb:Number(form.tgfb)||0 };
  ...
  return (
    <div className="page">
      <aside className="sidebar">
        ...
        <div className="nav">
          {[["dashboard","Дашборд",LayoutDashboard],["patients","Пациенты",UserRound],["analytics","Аналитика",BarChart3],["methodology","Методология",Microscope],["about","О платформе",FileText]].map(([k,l,Icon])=>(
            <button key={String(k)} className={`navBtn ${page===k ? "active" : ""}`} onClick={()=>setPage(String(k))}><Icon size={16}/>{l}</button>
          ))}
        </div>
        <div className="profile"><div style={{fontWeight:700,color:"#fff"}}>Пользователь</div><div style={{marginTop:6}}>{user.email}</div><div className="tiny" style={{marginTop:12}}>Платформа показывает ориентировочную оценку риска и не заменяет клиническое решение врача.</div></div>
      </aside>

      <main className="main"><div className="container">
        <div className="hero"><div className="heroGrid"><div><div className="heroBadge"><HeartPulse size={14}/>Corneal Graft Analytics</div><h1 className="heroTitle">Платформа для наблюдения после трансплантации роговицы</h1><div className="heroText">Интерфейс объединяет patient registry, ориентировочную стратификацию риска, динамику лабораторных маркеров и клинические комментарии в одной понятной панели.</div><div className="heroActions"><button className="btn outline" onClick={openNew}><Plus size={16}/>Новый пациент</button><button className="btn outline" onClick={()=>exportReport(selected)}><Download size={16}/>PDF report</button></div></div><div className="grid"><StatCard title="Пациентов в системе" value={stats.total} caption="Активные клинические записи" icon={Database}/><StatCard title="Высокий риск" value={stats.high} caption="Группа повышенного внимания" icon={ShieldAlert}/><StatCard title="Средняя оценка" value={`${stats.avg}%`} caption="Средний показатель по базе" icon={BrainCircuit}/></div></div></div>
            ...
            <div className="grid g3">
              <div className="card" style={{gridColumn:"span 2"}}>
                ...
                <div className="grid g2" style={{marginTop:18}}>
                  <div className="grid">
                    <div className="panel"><div style={{display:"flex",justifyContent:"space-between",gap:12}}><div><div className="muted small">Активный пациент</div><div style={{fontSize:26,fontWeight:850,marginTop:6,color:"#fff"}}>{selected.fullName}</div><div className="muted small">{selected.id} · {selected.age} лет · визит {selected.lastVisit}</div></div><div className="brandIcon"><UserRound size={20}/></div></div></div>
                    ...
                    <div className="card" style={{padding:18}}><div className="muted small">Patient Timeline</div><div className="timelineList">{selectedTimeline.map((item, idx)=><div key={idx} className="timelineItem"><div className="timelineDate">{item.date}</div><div><div className="timelineTitle">{item.title}</div><div className="small muted" style={{marginTop:4}}>{item.text}</div></div></div>)}</div></div>
                  </div>

                  <div className="grid">
                    <div className="panel"><div className="grid g2" style={{alignItems:"center"}}><RiskGauge value={selectedRisk.risk} label={selectedRisk.level} /><div className="grid"><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><span className="badge dark">{selectedRisk.level} риск</span><span className="badge outlineBadge">{selectedRisk.status}</span></div><div className="small muted">Risk score</div><div className="progress"><div className="bar" style={{width:`${selectedRisk.risk}%`}}/></div><div className="card" style={{padding:14, marginTop:8}}><div className="muted tiny">Biomarker Trend</div><div className="miniTrend">{selectedTrend.map((t, i)=><div key={i} className="miniItem"><span>{t.name}</span><strong>{t.trend}</strong></div>)}</div></div></div></div></div>
                    ...
                    <div className="card" style={{padding:18}}><div className="muted small">Risk Contributors</div><div className="factorList">{selectedContributors.map((c, i)=><div key={i} className="factorRow"><span>{c.label}</span><span className="factorScore">+{c.score}</span></div>)}</div></div>
                    <div className="callout"><div className="muted small">AI Clinical Interpretation</div><div style={{marginTop:8,color:"#fff"}}>{selectedSummary}</div></div>
                  </div>
                </div>
              </div>
          ...
          <div className="grid g3">
            <div className="card">
              ...
              <div className="list" style={{marginTop:18}}>
                {filteredPatients.map(patient=>{ const risk = calculateRisk(patient); return <button key={patient.id} className={`patientBtn ${selectedId===patient.id ? "active" : ""}`} onClick={()=>setSelectedId(patient.id)}><div style={{display:"flex",justifyContent:"space-between",gap:10}}><div><div style={{fontWeight:700}}>{patient.fullName}</div><div className="muted small">{patient.id} · {patient.surgeryType} · визит {patient.lastVisit}</div></div><RiskBadge risk={risk.risk}/></div><div className="muted small" style={{marginTop:8}}>{patient.notes}</div></button>; })}
              </div>
            </div>
            ...
            <div className="card" style={{gridColumn:"span 2"}}>
              ...
              {tab==="summary" && <div className="grid g3" style={{marginTop:18}}>
                ...
                ].map(([label,value])=><div key={String(label)} className="panel"><div className="muted small">{label}</div><div style={{marginTop:6,fontWeight:700,color:"#fff"}}>{value}</div></div>)}
                <div className="callout" style={{gridColumn:"1 / -1"}}><div className="muted small">Автоматический clinical summary</div><div style={{marginTop:8,color:"#fff"}}>{selectedSummary}</div></div>
              </div>}

              {tab==="markers" && <div className="grid" style={{marginTop:18}}>
                <div className="markerGrid">{[["IL-1", selected.il1],["IL-6", selected.il6],["TNF-α", selected.tnf],["VEGF", selected.vegf],["TGF-β", selected.tgfb]].map(([label,value])=><div key={String(label)} className="marker"><span className="markerTitle">{label}</span><div className="markerValue">{value}</div></div>)}</div>
                <div className="grid g2">
                  <div className="card" style={{padding:18}}><div className="muted small" style={{marginBottom:10}}>Динамика маркеров по визитам</div><div style={{height:300}}><ResponsiveContainer width="100%" height="100%"><LineChart data={trendData}><CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,.18)" /><XAxis dataKey="date" stroke="#94a3b8" /><YAxis stroke="#94a3b8" /><Tooltip /><Legend /><Line type="monotone" dataKey="IL1" name="IL-1" strokeWidth={2} /><Line type="monotone" dataKey="IL6" name="IL-6" strokeWidth={2} /><Line type="monotone" dataKey="TNFa" name="TNF-α" strokeWidth={2} /><Line type="monotone" dataKey="VEGF" name="VEGF" strokeWidth={2} /><Line type="monotone" dataKey="TGFB" name="TGF-β" strokeWidth={2} /></LineChart></ResponsiveContainer></div></div>
                  <div className="card" style={{padding:18}}><div className="muted small" style={{marginBottom:10}}>Risk History</div><div style={{height:300}}><ResponsiveContainer width="100%" height="100%"><LineChart data={trendData}><CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,.18)" /><XAxis dataKey="date" stroke="#94a3b8" /><YAxis stroke="#94a3b8" /><Tooltip /><Line type="monotone" dataKey="risk" name="Risk %" strokeWidth={3} /></LineChart></ResponsiveContainer></div></div>
                </div>
              </div>}
              ...
              {tab==="recommendations" && <div className="grid g2" style={{marginTop:18}}>
                ...
                <div className="grid">
                  <div className="card" style={{padding:18}}><div className="muted small">Risk Contributors</div><div className="factorList">{selectedContributors.map((c, i)=><div key={i} className="factorRow"><span>{c.label}</span><span className="factorScore">+{c.score}</span></div>)}</div></div>
                  <div className="callout"><div className="muted small">AI Explanation</div><div style={{marginTop:8,color:"#fff"}}>{selectedSummary}</div></div>
                </div>
              </div>}
              ...
              {tab==="report" && <div className="reportSheet" style={{marginTop:18}}>
                ...
                <div className="reportBlock" style={{marginTop:14}}><strong>Clinical summary</strong><div style={{marginTop:8}}>{selectedSummary}</div></div>
        ...
        {page==="analytics" && <div className="grid">
          <div className="grid g2">
            <div className="card"><h2>Распределение по уровням риска</h2><div className="muted small">Сводная стратификация базы пациентов</div><div style={{height:320,marginTop:10}}><ResponsiveContainer width="100%" height="100%"><BarChart data={riskChart}><CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,.18)" /><XAxis dataKey="name" stroke="#94a3b8" /><YAxis allowDecimals={false} stroke="#94a3b8" /><Tooltip /><Bar dataKey="value" radius={[10,10,0,0]} /></BarChart></ResponsiveContainer></div></div>
            <div className="card"><h2>Средние значения биомаркеров</h2><div className="muted small">Усреднённые лабораторные показатели по базе</div><div style={{height:320,marginTop:10}}><ResponsiveContainer width="100%" height="100%"><BarChart data={biomarkerChart}><CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,.18)" /><XAxis dataKey="marker" stroke="#94a3b8" /><YAxis stroke="#94a3b8" /><Tooltip /><Bar dataKey="value" radius={[10,10,0,0]} /></BarChart></ResponsiveContainer></div></div>
          </div>
          <div className="grid g2">
            <div className="card"><h2>Распределение по видам операций</h2><div className="muted small">Структура наблюдений по типам вмешательств</div><div style={{height:320,marginTop:10}}><ResponsiveContainer width="100%" height="100%"><BarChart data={surgeryChart}><CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,.18)" /><XAxis dataKey="name" stroke="#94a3b8" /><YAxis allowDecimals={false} stroke="#94a3b8" /><Tooltip /><Bar dataKey="value" radius={[10,10,0,0]} /></BarChart></ResponsiveContainer></div></div>
            <div className="card"><h2>Risk History</h2><div className="muted small">Пример динамики оценки риска</div><div style={{height:320,marginTop:10}}><ResponsiveContainer width="100%" height="100%"><LineChart data={trendData}><CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,.18)" /><XAxis dataKey="date" stroke="#94a3b8" /><YAxis stroke="#94a3b8" /><Tooltip /><Line type="monotone" dataKey="risk" name="Risk %" strokeWidth={3} /></LineChart></ResponsiveContainer></div></div>
          </div>
        </div>}

        {page==="methodology" && <div className="grid g3">
          <div className="card" style={{gridColumn:"span 2"}}><h2>Методологические принципы</h2><div className="muted small">Как интерпретировать интерфейс и расчётные блоки</div><div className="grid" style={{marginTop:18}}>
            {[
            ...
            ].map(([title,text])=><div key={String(title)} className="panel"><div style={{fontWeight:700,color:"#fff"}}>{title}</div><div className="small muted" style={{marginTop:6}}>{text}</div></div>)}
          </div></div>
          <div className="card"><h2>Новые улучшения v8</h2><div className="grid" style={{marginTop:18,gap:10}}><div className="factorRow"><span><WandSparkles size={16} style={{marginRight:8}}/> Risk Contributors</span><strong>New</strong></div><div className="factorRow"><span><AlertTriangle size={16} style={{marginRight:8}}/> Clinical Alert</span><strong>New</strong></div><div className="factorRow"><span><Activity size={16} style={{marginRight:8}}/> Risk History</span><strong>New</strong></div></div></div>
        </div>}

        {page==="about" && <div className="grid g2">
          <div className="card"><h2>О платформе</h2><div className="grid small muted" style={{marginTop:18}}><div>Corneal Risk Platform объединяет клинические данные пациента, ориентировочную стратификацию риска и аналитические панели в единой рабочей среде.</div><div>Интерфейс подходит для демонстрации концепции цифровой поддержки принятия решений в офтальмологии и для презентации логики развития такого продукта.</div><div>Важный принцип платформы — не подменять врача, а систематизировать сведения и делать клиническую картину более наглядной.</div></div></div>
          <div className="card"><h2>Дальнейшее развитие</h2><div className="grid small muted" style={{marginTop:18}}><div>• Backend и база данных</div><div>• Интеграция с клиническими системами</div><div>• Роли пользователей</div><div>• Реальные датасеты</div><div>• Внешняя валидация модели</div></div></div>
        </div>}
      </div></main>
      ...
      {modal && <div className="modalBg"><div className="modal">
        ...
        <div style={{display:"flex",justifyContent:"end",gap:10,marginTop:18}}><button className="btn outline" onClick={()=>setModal(false)}>Отмена</button><button className="btn" onClick={save}><ClipboardList size={16}/>Сохранить пациента</button></div>
      </div></div>}
    </div>
  );
}
